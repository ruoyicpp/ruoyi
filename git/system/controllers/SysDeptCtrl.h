...
public:
    ...
    void getById(const drogon::HttpRequestPtr &req, std::function<void(const drogon::HttpResponsePtr &)> &&cb, long deptId) {
        ...
        if (!res.ok() || res.rows() == 0) { RESP_ERR(cb, "不允许修改父节点"); return; }
    ...
    void add(const drogon::HttpRequestPtr &req, std::function<void(const drogon::HttpResponsePtr &)> &&cb) {
        ...
        if (exist.ok() && exist.rows() > 0) { RESP_ERR(cb, "新增部门'" + deptName + "'失败，部门名称已存在"); return; }
        ...
        db.execParams(
            ...
            {parentId, ancestors, deptName,
             ...
             GET_USER_NAME(req)});
        LOG_OPER(req, "部门管理", BusinessType::INSERT);
        RESP_MSG(cb, "操作成功");
    }
    ...
    void edit(const drogon::HttpRequestPtr &req, std::function<void(const drogon::HttpResponsePtr &)> &&cb) {
        ...
        if (deptId == parentId) { RESP_ERR(cb, "修改部门失败，上级部门不能选择自己"); return; }
        auto& db = DatabaseService::instance();
        ...
        if (!oldRes.ok() || oldRes.rows() == 0) { RESP_ERR(cb, "不允许修改父节点"); return; }
        std::string oldAncestors = oldRes.str(0, 0);
        // 计算新的 ancestors：父级的 ancestors + "," + 父级ID
    ...
    void remove(const drogon::HttpRequestPtr &req, std::function<void(const drogon::HttpResponsePtr &)> &&cb, long deptId) {
        ...
        if (child.ok() && child.rows() > 0 && child.intVal(0, 0) > 0) { RESP_ERR(cb, "存在下级部门,不允许删除"); return; }
        auto users = db.queryParams("SELECT COUNT(*) FROM sys_user WHERE dept_id=$1 AND del_flag='0'", {sid});
        if (users.ok() && users.rows() > 0 && users.intVal(0, 0) > 0) { RESP_ERR(cb, "部门存在用户,不允许删除"); return; }
        db.execParams("UPDATE sys_dept SET del_flag='2' WHERE dept_id=$1", {sid});
        LOG_OPER_PARAM(req, "部门管理", BusinessType::REMOVE, sid);
        RESP_MSG(cb, "操作成功");
    }

private:
        // list字段: dept_id(0),parent_id(1),ancestors(2),dept_name(3),order_num(4),leader(5)
