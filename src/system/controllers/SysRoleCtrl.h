...
public:
    ...
    void getById(const drogon::HttpRequestPtr &req, std::function<void(const drogon::HttpResponsePtr &)> &&cb, long id) {
        ...
        auto res = DatabaseService::instance().queryParams(
            ...
            {std::to_string(id)});
        if (!res.ok() || res.rows() == 0) { RESP_ERR(cb, "角色不存在"); return; }
    ...
    void add(const drogon::HttpRequestPtr &req, std::function<void(const drogon::HttpResponsePtr &)> &&cb) {
        ...
        if (exist.ok() && exist.rows() > 0) { RESP_ERR(cb, "新增角色'" + roleName + "'失败，角色名称已存在"); return; }
        auto exist2 = db.queryParams("SELECT role_id FROM sys_role WHERE role_key=$1 AND del_flag='0' LIMIT 1", {roleKey});
        if (exist2.ok() && exist2.rows() > 0) { RESP_ERR(cb, "新增角色'" + roleName + "'失败，角色权限已存在"); return; }
        ...
        auto ins = db.queryParams(
            ...
            {roleName, roleKey,
             ...
             GET_USER_NAME(req)});
        if (!ins.ok() || ins.rows() == 0) { RESP_ERR(cb, "新增角色失败"); return; }
        ...
        LOG_OPER(req, "角色管理", BusinessType::INSERT);
        RESP_MSG(cb, "操作成功");
    }
    ...
    void edit(const drogon::HttpRequestPtr &req, std::function<void(const drogon::HttpResponsePtr &)> &&cb) {
        ...
        if (SecurityUtils::isAdminRole(roleId)) { RESP_ERR(cb, "不允许操作超级管理员角色"); return; }
        auto& db = DatabaseService::instance();
            ...
            if (allTokensRes.ok()) {
                for (int t = 0; t < allTokensRes.rows(); ++t) {
                    ...
                    auto roleRes = db.queryParams(
                        ...
                        {std::to_string(uid)});
                    cachedUser->permissions.clear();
                    cachedUser->roles.clear();
                    if (permsRes.ok())
                        for (int p = 0; p < permsRes.rows(); ++p)
                            cachedUser->permissions.push_back(permsRes.str(p, 0));
                    if (roleRes.ok())
                        for (int r2 = 0; r2 < roleRes.rows(); ++r2)
                            cachedUser->roles.push_back(roleRes.str(r2, 0));
                    TokenCache::instance().update(tokenKey, *cachedUser);
                }
            }
        }
    ...
    void remove(const drogon::HttpRequestPtr &req, std::function<void(const drogon::HttpResponsePtr &)> &&cb, const std::string &ids) {
        ...
        for (auto &idStr : splitIds(ids)) {
            ...
            if (cnt.ok() && cnt.rows() > 0 && cnt.intVal(0, 0) > 0) { RESP_ERR(cb, "角色已分配，不允许删除"); return; }
        ...
        LOG_OPER_PARAM(req, "角色管理", BusinessType::REMOVE, ids);
        RESP_MSG(cb, "操作成功");
    }
    ...
    void cancelAuthUser(const drogon::HttpRequestPtr &req, std::function<void(const drogon::HttpResponsePtr &)> &&cb) {
        ...
        auto body = req->getJsonObject();
        if (!body) { RESP_ERR(cb, "请求体格式错误"); return; }
        // 前端 vue-router query 传来的 roleId 可能是 string 或 number，统一处理
        auto jsonToStr = [](const Json::Value& v) -> std::string {
            if (v.isString()) return v.asString();
            return std::to_string(v.asInt64());
        };
        std::string roleId = jsonToStr((*body)["roleId"]);
        std::string userId = jsonToStr((*body)["userId"]);
        if (roleId.empty() || userId.empty()) { RESP_ERR(cb, "参数缺失"); return; }
        DatabaseService::instance().execParams(
            "DELETE FROM sys_user_role WHERE role_id=$1 AND user_id=$2", {roleId, userId});
        LOG_OPER(req, "角色管理", BusinessType::GRANT);
        RESP_MSG(cb, "操作成功");
    }
    ...
    void exportData(const drogon::HttpRequestPtr &req, std::function<void(const drogon::HttpResponsePtr &)> &&cb) {
        ...
        Json::Value rows(Json::arrayValue);
        if (res.ok()) for (int i = 0; i < res.rows(); ++i) rows.append(roleRowToJson(res, i));
        LOG_OPER(req, "角色管理", BusinessType::EXPORT);
        auto csv = CsvUtils::toCsv(rows, {
            {"角色编号","roleId"},{"角色名称","roleName"},{"角色权限字符","roleKey"},
            {"显示顺序","roleSort"},{"数据范围","dataScope"},{"状态","status"},
...
private:
        // 查询字段: role_id,role_name,role_key,role_sort,data_scope,status,remark,create_time
        // list字段: role_id(0),role_name(1),role_key(2),role_sort(3),data_scope(4),status(5)
