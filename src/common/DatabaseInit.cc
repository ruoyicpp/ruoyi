...
std::vector<std::string> DatabaseInit::getCreateTableSqls() {
    return {
        ...
        R"(CREATE TABLE IF NOT EXISTS sys_config (
            ...
            config_key   VARCHAR(100) NOT NULL DEFAULT '' UNIQUE,
        ...
        // AI 智能助手 — 会话表
        // -------------------------------------------------------
        R"(CREATE TABLE IF NOT EXISTS ai_chat_session (
        ...
        // AI 智能助手 — 消息表
        // -------------------------------------------------------
        R"(CREATE TABLE IF NOT EXISTS ai_chat_message (
        ...
        R"(CREATE INDEX IF NOT EXISTS idx_ai_msg_session ON ai_chat_message(session_id))",

        // -------------------------------------------------------
        // sys_token — Token 持久化表（重启后恢复在线会话）
        // -------------------------------------------------------
        R"(CREATE TABLE IF NOT EXISTS sys_token (
        ...
        // 表单构建配置表
        // -------------------------------------------------------
        R"(CREATE TABLE IF NOT EXISTS tool_form_conf (
        ...
        // -------------------------------------------------------
        // 设备绑定表（硬件指纹 + RSA 签名，单行 id=1）
        // 重置方法：DELETE FROM sys_device_binding WHERE id=1
        // -------------------------------------------------------
        R"(CREATE TABLE IF NOT EXISTS sys_device_binding (
...
std::vector<std::string> DatabaseInit::getInitDataSqls() {
    return {
        ...
        R"(INSERT INTO sys_menu (menu_id,menu_name,parent_id,order_num,path,component,query,is_frame,is_cache,menu_type,visible,status,perms,icon,create_by,create_time,remark) VALUES
           (4,'若依官网',0,4,'http://ruoyi.vip','InnerLink','','0','0','C','0','0','','guide','admin',NOW(),'若依官网地址') ON CONFLICT (menu_id) DO NOTHING)",
        R"(UPDATE sys_menu SET menu_type='C', component='InnerLink', is_frame='0'
           WHERE menu_id=4)",
        R"(INSERT INTO sys_menu (menu_id,menu_name,parent_id,order_num,path,component,query,is_frame,is_cache,menu_type,visible,status,perms,icon,create_by,create_time,remark) VALUES
           (2000,'AI智能助手',0,5,'http://localhost:5001','InnerLink','','0','0','C','0','0','','robot','admin',NOW(),'AI聊天助手') ON CONFLICT (menu_id) DO NOTHING)",
        ...
        R"(INSERT INTO sys_menu (menu_id,menu_name,parent_id,order_num,path,component,query,is_frame,is_cache,menu_type,visible,status,perms,icon,create_by,create_time,remark) VALUES
           (2003,'修改',2001,2,'','','','1','0','F','0','0','system:emailConfig:edit','#','admin',NOW(),'') ON CONFLICT (menu_id) DO NOTHING)",
        R"(INSERT INTO sys_menu (menu_id,menu_name,parent_id,order_num,path,component,query,is_frame,is_cache,menu_type,visible,status,perms,icon,create_by,create_time,remark) VALUES
           (108,'日志管理',1,10,'log','','','1','0','M','0','0','','log','admin',NOW(),'日志管理菜单') ON CONFLICT (menu_id) DO NOTHING)",
        R"(INSERT INTO sys_menu (menu_id,menu_name,parent_id,order_num,path,component,query,is_frame,is_cache,menu_type,visible,status,perms,icon,create_by,create_time,remark) VALUES
        ...
        R"(INSERT INTO sys_menu (menu_id,menu_name,parent_id,order_num,path,component,query,is_frame,is_cache,menu_type,visible,status,perms,icon,create_by,create_time,remark) VALUES
           (118,'网站信息查询',3,4,'websiteInfo','tool/websiteInfo/index','','1','0','C','0','0','tool:websiteInfo:query','search','admin',NOW(),'查询网站IP/标题/图标等基础信息') ON CONFLICT (menu_id) DO NOTHING)",
        ...
        R"(INSERT INTO sys_role_menu (role_id,menu_id) VALUES
           ...
           (2,109),(2,110),(2,111),(2,112),(2,113),(2,114),(2,115),(2,116),(2,117),(2,118),
           (2,500),(2,501),
        ...
        R"(INSERT INTO sys_config (config_id,config_name,config_key,config_value,config_type,create_by,create_time,remark)
           ...
           ON CONFLICT (config_key) DO NOTHING)",
        R"(INSERT INTO sys_config (config_id,config_name,config_key,config_value,config_type,create_by,create_time,remark)
           VALUES (7,'账号自助-注册用户默认角色','sys.account.initRoleId','','Y','admin',NOW(),'注册用户自动分配的角色ID，留空则不分配角色')
           ON CONFLICT (config_key) DO NOTHING)",
        R"(INSERT INTO sys_config (config_id,config_name,config_key,config_value,config_type,create_by,create_time,remark)
           VALUES (12,'随机视频-是否启用','sys.video.enabled','true','Y','admin',NOW(),'是否开启随机视频功能（true=开启，false=关闭）')
           ON CONFLICT (config_key) DO NOTHING)",
        R"(INSERT INTO sys_config (config_id,config_name,config_key,config_value,config_type,create_by,create_time,remark)
           VALUES (13,'账号自助-忘记密码开关','sys.account.forgotPwdEnabled','true','Y','admin',NOW(),'是否开启忘记密码功能（true=开启，false=关闭）')
           ON CONFLICT (config_key) DO NOTHING)",
        R"(INSERT INTO sys_config (config_id,config_name,config_key,config_value,config_type,create_by,create_time,remark)
        ...
        R"(INSERT INTO sys_config (config_id,config_name,config_key,config_value,config_type,create_by,create_time,remark)
           ...
           ON CONFLICT (config_key) DO NOTHING)",
        R"(DELETE FROM sys_config WHERE config_key='sys.subprocess.vault')",
        R"(INSERT INTO sys_config (config_id,config_name,config_key,config_value,config_type,create_by,create_time,remark)
           VALUES (23,'随机视频-功能开关','sys.video.enabled','true','Y','admin',NOW(),'true=开启随机视频功能，false=关闭')
           ON CONFLICT (config_key) DO NOTHING)",
        R"(INSERT INTO sys_config (config_id,config_name,config_key,config_value,config_type,create_by,create_time,remark)
        ...
        // schema patch: 已有数据库补加 config_key 唯一约束
        "DO $$ BEGIN IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname='sys_config_config_key_uq') THEN ALTER TABLE sys_config ADD CONSTRAINT sys_config_config_key_uq UNIQUE (config_key); END IF; END $$",
...
static std::string pgDdlToSqlite(const std::string& in) {
    ...
    s = sqReplAll(s, "DEFAULT NOW()", "DEFAULT (datetime('now'))");
    s = sqReplAll(s, "DEFAULT now()", "DEFAULT (datetime('now'))");
...
std::vector<std::string> DatabaseInit::getSqliteInitDataSqls() {
    ...
    for (auto& sql : getInitDataSqls()) {
        ...
        if (sql.size() >= i + 2 &&
            std::toupper((unsigned char)sql[i])   == 'D' &&
            std::toupper((unsigned char)sql[i+1]) == 'O') continue;
